...
_DESIGN_GUIDANCE = (
    "\n\nПРИНЦИПЫ РАБОТЫ (система автоматического проектирования):\n"
    "• ВЫВОДИ параметры САМ. Химический состав сырья, подбор и смешение глин, нормы "
    "ввода отощителей и добавок — определяй из ОТЧЁТОВ ЛАБОРАТОРИИ (материалы проекта). "
    "Режимы сушки/обжига, подбор оборудования и приводов, энергопотребление, площади и "
    "планировку цехов, нормы потерь, окупаемость — РАССЧИТЫВАЙ по входным данным и "
    "инженерным методикам.\n"
    "• У пользователя спрашивай ТОЛЬКО то, без чего непонятна цель проекта (вид продукции, "
    "объём). Если для расчёта не хватает данных, которые должны быть в отчёте лаборатории "
    "или материалах — укажи, какой документ нужно приложить. Не выдумывай значения.\n"
    "• Отвечай КРАТКО: конкретный результат — цифры, таблицы. Без вступлений и рассуждений.\n"
    "• База знаний — ПРИМЕРЫ и ориентиры, не строгие требования. Присланные материалы "
    "проекта (готовый проект, отчёт лаборатории) — ПРИОРИТЕТ.\n"
    "/no_think"
)
...
async def _stream_text(messages: list[BaseMessage]) -> str:
    ...
    """
    # /no_think в последнем ходе надёжно отключает «думанье» qwen3.
    # Копируем сообщение (не мутируем общую историю, чтобы метка не сохранилась).
    _brevity = (
        "\n\n(Отвечай максимально кратко и по делу: только итоговый результат — "
        "список/таблица/цифры. Без вступлений, без рассуждений вслух. /no_think)"
    )
    msgs = list(messages)
    if msgs:
        last = msgs[-1]
        try:
            msgs[-1] = last.model_copy(update={"content": (last.content or "") + _brevity})
        except Exception:
            pass

    parts: list[str] = []
    async for chunk in _llm(streaming=True).astream(msgs):
        if chunk.content:
...
def _strip_thinking(text: str) -> str:
    """Убрать «размышления» qwen3 (рассуждение перед закрывающим </think>)."""
    low = text.lower()
    j = low.rfind("</think>")
    if j != -1:
        return text[j + len("</think>"):].lstrip()
    import re as _re
    return _re.sub(r"<think>.*?</think>", "", text, flags=_re.DOTALL | _re.IGNORECASE).lstrip()
